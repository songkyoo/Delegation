...
{
    public static ImmutableArray<GenerationAnalysisResult<ExposeGenerationContext>> CreateAll(
    ...
    )
    {
        ...
        {
            return ImmutableArray<GenerationAnalysisResult<ExposeGenerationContext>>.Empty;
        }

        var builder = ImmutableArray.CreateBuilder<GenerationAnalysisResult<ExposeGenerationContext>>(context.Attributes.Length);
    ...
    private static GenerationAnalysisResult<ExposeGenerationContext> Create(AttributeData attributeData, ISymbol declaredSymbol)
    {
        var constructorArguments = attributeData.ConstructorArguments;
        var includeBaseTypes = attributeData
            .NamedArguments
            .FirstOrDefault(static argument => argument.Key == nameof(ExposeAttribute.IncludeBaseTypes))
            .Value
            .Value is true;
    ...
    )
    {
        ...
        foreach (var (value, location) in AttributeArgumentReader.ReadStringArguments(attributeData, parameterName))
        {
            ...
            builder.Add(Diagnostic.Create(
                descriptor: ExposeDiagnostics.ExposeMemberNameNotFoundRule,
                location: location,
